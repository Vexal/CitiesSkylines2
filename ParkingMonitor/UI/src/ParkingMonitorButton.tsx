						...
						<div style={{marginTop:"10rem"} }>
							...
							<div className={styles.parkingMonitorLotList}>
								{this.state.parkingBindings?.districtOrder.map((district, i) => {
									...
									return (
										<div key={district}>
											<div style={{ color: "rgba(55,195,241,1)", paddingTop:"10rem" }}>
												<span>{district}</span>
											</div>
											<div>
												{districtLots.slice(0, Math.min(10, districtLots.length)).map((lot, lotInd) => <div style={{display:"flex", width:"420rem"}} key={lot.key }>
													...
													<span style={{ width: "130rem", fontSize:"15rem" }} className={styles.tbl}>
														{lot.key}
													</span>
													<span style={{ width: "20rem" }} className={styles.tbl}>
														{lot.count }
	...
	<DataItem key={lotInd}
		...
		cb={(k: String) => {
			trigger(MOD_NAME, "selectLot", k);
										...
										<table>
											<tbody>
												{districtLots.slice(0, Math.min(10, districtLots.length)).map((lot, lotInd) => <tr>
													<td style={{ width: "20rem" }} className={styles.tbl}>
														{lotInd + 1 }
													</td>
													<td style={{ width: "300rem" }} className={styles.tbl}>
														{lot.name}
													</td>
												</tr>)}
											</tbody>
