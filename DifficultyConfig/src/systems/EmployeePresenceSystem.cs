...
{
	internal partial class EmployeePresenceSystem : GameSystemBase
	{
		private DifficultySettings settings;
		private CitySystem citySystem;
		private EntityQuery buildingQuery;
		private EntityQuery deletionQuery;
		...
		protected override void OnCreate()
		{
			...
			this.deletionQuery = GetEntityQuery(ComponentType.ReadWrite<DeleterTimer>());
		...
		protected override void OnUpdate()
		{
			if (EntityManager.TryGetComponent(this.citySystem.City, out CollapsingCity collapsingCity))
			{
				...
				if (this.frameCount % 200 == 0)
				{
					...
					for (int i = 0; i < 10 && i < maybeDeletes.Length; ++i)
					{
